...
void COBTManager__get_current_obt(const termina__event_t * const termina__ev, void * const termina__this, MissionOBT * const current_obt) {
    ...
    #line 57 "src/resources/obt_manager.fin"
    uint32_t fine_time_increment = (uint32_t)self->ref_obt.finetime;

    #line 58 "src/resources/obt_manager.fin"
    fine_time_increment = fine_time_increment + (uint32_t)((uint32_t)(interval.tv_usec * 1000U) / 15259U);

    #line 60 "src/resources/obt_manager.fin"
