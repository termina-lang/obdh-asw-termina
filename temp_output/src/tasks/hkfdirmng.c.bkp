...
Result HK_FDIR_Manager__pus_serv5_build_event_list_tms(HK_FDIR_Manager * const self) {
    ...
    for (size_t i = 0; i < max_num_events && i < self->event_list.num_events; i = i + 1) {
        ...
        extract_event(&self->event_list, &event_info);
        ...
        if (event_info.__variant == None) {
            ...
            if (is_RID_enabled(__Some.__0.ev_RID, self->RID_enable_config)) {
...
void HK_FDIR_Manager__pus_serv3_build_tm_3_25(const HK_FDIR_Manager * const self,
    ...
    tm_packet_header.packet_length = 4 + (uint16_t)(2 * (uint16_t)self->hk_config_table[index].num_params);
    ...
    p_tm_descriptor->tm_bytes[10] = self->hk_config_table[index].SID;

    size_t PID = 0;

    uint16_t SDP_value = 0;

    for (size_t j = 0; j < max_params_per_SID && j < (size_t)self->hk_config_table[index].num_params; j = j + 1) {
